...
local buttonIsDownA = false
local timer = nil
local buttonHoldTime = 1

local function OnExpired( self, runSecondryCmd )
    if buttonIsDownA then
       buttonIsDownA = false
       local Modifiers = {}
...
Checkbox = Class(Bitmap)
{
    ...
    SetAllowAButtonHold = function( self, allow )
        self._allowAButtonHold = allow
    end,
    ...
    HandleEvent = function(self, event)
        ...
        elseif event.Type == 'ButtonPress' or event.Type == 'ButtonDClick' then
            ...
            eventHandled = true
        elseif event.Type == 'JoystickButtonPress' or event.Type == 'JoystickButtonRelease' then
            ...
            if navigateTo != nil then
                ...
                eventHandled = true
            elseif event.Type == 'JoystickButtonPress' and event.JoystickButton == 'A' then                
------   -JB MOD for button held
                buttonIsDownA = true
                if self._allowAButtonHold then
    				HoldFunc = function()
    				    OnExpired( self )
    				end
                    if not timer then
...
------
                ...
                eventHandled = true
            ---- JB add for (HOLD) on a command to toggle.. ie: Dive -> auto Dive on/off
            elseif event.Type == 'JoystickButtonRelease' and event.JoystickButton == 'A' then
                if buttonIsDownA then
                    self:OnClick( event.Modifiers )
                end
                buttonIsDownA = false
