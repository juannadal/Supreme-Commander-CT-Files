    ...
    __init = function(self, parent, axis, debugname)
        ...
        self.HandleEvent = function(ctrl, event)
            ...
            if event.Type == 'JoystickButtonPress' then
                if axis == ScrollAxis.Vert then
                    if event.JoystickButton == 'DPadUp' and self.UpButton then
                        self.UpButton:OnClick()
                        eventHandled= true
                    elseif event.JoystickButton == 'DPadDown' and self.DownButton then
                        self.DownButton:OnClick()
                        eventHandled= true
                    end
                else
                    if event.JoystickButton == 'DPadLeft' and self.UpButton then
                        self.UpButton:OnClick()
                        eventHandled= true
                    elseif event.JoystickButton == 'DPadRight' and self.DownButton then
                        self.DownButton:OnClick()
                        eventHandled= true
                    end
                end
