...
Cursor = Class(moho.cursor_methods) {
	__init = function(self, useTexturedDeviceCursor, defaultTexture, defaultHotspotX, defaultHotspotY )
		_c_CreateCursor(self, nil, useTexturedDeviceCursor)
		...
		self._filename = import('/lua/system/lazyvar.lua').Create()
