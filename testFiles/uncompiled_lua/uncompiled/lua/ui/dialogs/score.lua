...
local tabs = {
    ...
    },
    {
        ...
        scoreKey = 'general',
        columns = {
          {title = "Score", scoreKey = 'score',},
          {title = "Kills", scoreKey = 'kills',},
          {title = "Built", scoreKey = 'built',},
          {title = "Losses", scoreKey = 'lost',},
          {title = "Mass", scoreKey = 'mass',},
          {title = "Energy", scoreKey = 'energy',},
        },
        types = {
            {title = "Count", scoreKey = 'count',},
            {title = "Mass Value", scoreKey = 'mass',},
            {title = "Energy Value", scoreKey = 'energy',},
        },
        grid = true,
    },
    {
        ...
        scoreKey = 'units',
        columns = {
            {title = "CDR", scoreKey = 'cdr',},
            {title = "Land", scoreKey = 'land',},
            {title = "Naval", scoreKey = 'naval',},
            {title = "Air", scoreKey = 'air',},
            {title = "Structures", scoreKey = 'structures',},
            {title = "Experimental", scoreKey = 'experimental',},
        },
        types = {
            {title = "Kills", scoreKey = 'kills',},
            {title = "Built", scoreKey = 'built',},
    ...
    },
    {
        ...
        scoreKey = 'resources',
        columns = {
            {title = "Mass Collected", scoreKey = 'massin',},
            {title = "Mass Spent", scoreKey = 'massout',},
            {title = "Mass Wasted", scoreKey = 'massover',},
            {title = "Energy Collected", scoreKey = 'energyin',},
            {title = "Energy Spent", scoreKey = 'energyout',},
            {title = "Energy Wasted", scoreKey = 'energyover',},
        },
        types = {
            {title = "Total", scoreKey = 'total',},
            {title = "Rate", scoreKey = 'rate',},
        },
        grid = true,
    },
}
    ...
    if showCampaign then
        ...
        if opData.opMovies.postOpMovies[successKey] then
            ...
            local creditData = {
                ...
                illuminate = {vid = '/movies/Credits_Aeon.sfd', sfx = 'X_FMV_Aeon_Credits', voice = 'SCX_Aeon_Credits_VO', subtitles = subtitleSource.SCX_Aeon_Credits_VO.captions},
            }
            local movies = {
...
function CreateSkirmishScreen(victory, showCampaign, operationVictoryTable)
    ...
    bg.continueBtn = UIUtil.CreateButtonStd(bg, '/scx_menu/large-no-bracket-btn/large', "Continue", 22, 2, 0, "UI_Menu_MouseDown", "UI_Opt_Affirm_Over")
    ...
    bg.continueBtn.OnClick = function(self, modifiers)
        ...
        if HasCommandLineArg("/online") then
            ExitApplication()
        else
            ExitGame()
        end
    ...
    if showCampaign and not operationVictoryTable.success then
        bg.continueBtn.label:SetText('Skip')
        bg.continueBtn.HandleEvent = bg.continueBtn.oldHandleEvent
        Tooltip.AddButtonTooltip(bg.continueBtn, 'CampaignScore_Skip')
        -- set controls that are global to the dialog
        bg.restartBtn = UIUtil.CreateButtonStd(bg, '/scx_menu/large-no-bracket-btn/large', "Restart", 22, 2, 0, "UI_Menu_MouseDown", "UI_Opt_Affirm_Over")
    ...
    local elapsedTimeLabel = UIUtil.CreateText(bg, "Game Time:", 16, UIUtil.bodyFont)
    ...
    bg.replayButton = UIUtil.CreateButtonStd(bg, '/scx_menu/medium-no-br-btn/medium-uef', "", 14, 2)
...
--            teamText:SetText("Team")
        ...
        elseif tabData.button == "campaign" then
            ...
            local prefix = {
                ...
                Illuminate = {texture = '/icons/comm_seraphim.dds', cue = 'UI_Comm_SER'},
            ...
            local opScoreLabel = UIUtil.CreateText(scoreGroup, "Operation Score", 14, UIUtil.bodyFont)
            ...
            if hasPrimaries then
                table.insert(sortedObjectives, {title = "Primary Objectives", type = 'header'})
            ...
            if hasSecondaries then
                table.insert(sortedObjectives, {title = "Secondary Objectives", type = 'header'})
                for i, v in tempObjectives do
            ...
            objContainer.CalcVisible = function(self)
                local function SetTextLine(line, data, lineID)
                    if data.type == 'header' then
                        ...
                        line.result:SetText(data.complete or "No action text")
        ...
        if operationVictoryTable then
            if operationVictoryTable.success then
                victoryString = "Operation Successful"
            else
                victoryString = "Operation Failed"
            end
        else
            if victory != nil then
                if victory then
