...
--local OnlineProvider = import('/lua/ui/multiplayer/onlineprovider.lua')
...
function StartFrontEndUI()
    ...
    UIFrontEndInit()
	if not UseGamepad() and not UseNewFrontend() then
...
function StartGameUI()
	...
	-- Play the game start music cue (stops the main_menu one)
	ActivateMusicCue('Game_Start')
...
function UpdateDisconnectDialog()
        WARN('WARNING: UpdateDisconnectDialog is being referenced and is no longer supported - bfricks 1/12/10')
end
...
function NoteGameOver()
    ...
    if SessionIsReplay() then
        ...
        import('/lua/ui/game/score.lua').ArmyAnnounce(1, 'Replay over.')
    else
        import('/lua/ui/game/score.lua').ArmyAnnounce(1, 'Game over.')
    end
end
