...
WorldViewParams = {
	ui_SelectTolerance = 7.0,			-- Sets the unit click tolerance (in pixels) for selection.  Increase this to make units have a larger selection box.
	ui_DisableCursorFixing = false,		-- Allows you to toggle the cursor fixing functionality that is used for the mouse-controlled camera spinning/scrolling
	ui_ExtractSnapTolerance = 4.0,		-- Sets the extraction unit 'snap-to' tolerance (in meters) for building.  Increase this to make it easier to auto-snap to extraction sites.
	ui_MinExtractSnapPixels = 10,		-- Allows us to put a pixel cap on the snap tolerance (in case we are zoomed out relatively far)
	ui_MaxExtractSnapPixels = 1000,		-- Allows us to put a pixel cap on the snap tolerance (in case we are zoomed in close)
	ui_ZoomLowerBound = 400.0,			-- Camera must be zoomed in more than this to allow spinning
	cam_AllowPivot = true,				-- Allow camera to spin with space bar held down
	cam_AutoResetPivot = false,			-- Camera reverts to normal view when space bar is let go
}
        ...
        if table.getn(validAttackersSelection) == 1 then
            if EntityCategoryContains(categories.SHOWATTACKRETICLE, validAttackersSelection[1]) then
                attackReticleSize = validAttackersSelection[1]:GetBlueprint().Display.TacticalReticleSize or
                    ...
                    (validAttackersSelection[1]:GetBlueprint().Weapon[1].DamageRadius or 1) * 2
            end
...
WorldView = Class(moho.UIWorldView, Control) {
    ...
    OnUpdateCursor = function(self)
		-- Gamepad does not need to update the cursor.
		if UseGamepad() then
			return
		end
        ...
        local mode, modeData = import('/lua/ui/game/commandmode.lua').GetCommandMode()
        ...
        if mode == "order" then
            ...
            if self:ShowConvertToPatrolCursor() then
                ...
                if DecalFunctions[modeData.name] then
                    showInvalidTargetCursor, newDecalTexture, newScale = DecalFunctions[modeData.name]()
                ...
                elseif modeData.cursor then
                    self.Cursor = {UIUtil.GetCursor(modeData.cursor)}
                else
                    self.Cursor = {UIUtil.GetCursor(modeData.name)}
                end
            end
        ...
        elseif self:HasHighlightCommand() then
            ...
            if order then
                ...
                if order == "RULEUCC_Move" then                
					if ( not UICommandsHasFocus() and self:UICheckSelectionPathableState() == false ) then
						self.Cursor = {UIUtil.GetCursor('MOVE_INVALID')}
					else						
						self.Cursor = nil
					end
                else
            ...
            if StretchCursorHasEnemyTarget() then
				self.Cursor = {UIUtil.GetCursor("ATTACK_GAMEPAD")}
			end
    ...
    Register = function(self, cameraName, disableMarkers, displayName, order)
        ...
        --[[ GV 2/19/09: SetMaxZoomMult set a member that wasn't being used anywhere.
