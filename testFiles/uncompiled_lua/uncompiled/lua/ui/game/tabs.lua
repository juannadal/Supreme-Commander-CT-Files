...
function ToggleTab(tabID)
    --$$$ This check is mainly to deal with F10 call in game while we still have the old UI active
	if tabID == "main" then
		UIActivateMain();
		return
	end
...
function OnPause(state, pausedBy, lTimeoutsRemaining, isOwner)
	WARN('WARNING: OnPause is being referenced and is no longer supported - bfricks 1/12/10')
end
...
function CreateScreenGlow()
    ...
    pauseGlow.Top.OnFrame = function(self, delta)
        ...
        --$$$ ONLY FOR E3. We want to be full bright on pause
        --self:SetAlpha(newAlpha, true)
    end
