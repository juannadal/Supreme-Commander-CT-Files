...
function StartCountdown(entryData)
    ...
    for _, viewControl in views do
        ...
        timers[view._cameraName][entryData.entity] = UIUtil.CreateText(view, '5', 24, UIUtil.titleFont)
        ...
        text.UpdatePosition = function(self)
            ...
            self.Top:Set(function() return view.Top() + coords[2] - 50 end)
        end
        text.OnFrame = function(self, delta)
            self.time = self.time + delta
            ...
            if GetCamera(view._cameraName):GetTargetZoom() > 150 then
                self:Hide()
