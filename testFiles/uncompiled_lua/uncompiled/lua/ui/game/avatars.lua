...
function CreateIdleTab(unitData, id, expandFunc)
    ...
    bg.Update = function(self, units)
        ...
        if self.id == 'engineer' then
			...
			------------------------------------------------------------------------------
			-- SC2: temp resolution for tracking idle builiders
            sortedUnits[5] = EntityCategoryFilterDown(categories.SC2DONOTUSE, self.allunits)
            sortedUnits[4] = EntityCategoryFilterDown(categories.SC2DONOTUSE, self.allunits)
            sortedUnits[3] = EntityCategoryFilterDown(categories.SC2DONOTUSE, self.allunits)
            sortedUnits[2] = EntityCategoryFilterDown(categories.SC2DONOTUSE, self.allunits)
            sortedUnits[1] = EntityCategoryFilterDown(categories.ENGINEER, self.allunits)
			------------------------------------------------------------------------------
...
function CreateIdleEngineerList(parent, units)
    ...
    group.Update = function(self, unitData)
		...
		------------------------------------------------------------------------------
		-- SC2: temp resolution for tracking idle builiders
        engineers[5] = EntityCategoryFilterDown(categories.SC2DONOTUSE, unitData)
        engineers[4] = EntityCategoryFilterDown(categories.SC2DONOTUSE, unitData)
        engineers[3] = EntityCategoryFilterDown(categories.SC2DONOTUSE, unitData)
        engineers[2] = EntityCategoryFilterDown(categories.SC2DONOTUSE, unitData)
        engineers[1] = EntityCategoryFilterDown(categories.ENGINEER, unitData)
		------------------------------------------------------------------------------
...
function AvatarUpdate()
    ...
    local currentFaction = GetArmiesTable().armiesTable[GetFocusArmy()].faction + 1
...
function FocusArmyChanged()
	--WARN('WARNING: FocusArmyChanged is being referenced and is no longer supported - bfricks 1/12/10')
end
