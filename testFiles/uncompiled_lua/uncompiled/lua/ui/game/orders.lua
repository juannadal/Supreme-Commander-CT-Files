...
local function BuildInitFunction( orderInfo, control, unitList)
...
local function DiveInitFunction( orderInfo, control, unitList)
...
local function PauseInitFunction( orderInfo, control, unitList)
...
local function AbilityButtonInit( orderInfo, control, unitList )
	control.abilityIndex = orderInfo.abilityIndex
    control.commandType = orderInfo.commandType
end

local function AbilityButtonBehavior(self, modifiers)
    -- test if we're something to see what kind of command we should be
    if self.commandType == 'EACT_CommandMode' then
        -- if we're checked, end the current command mode, otherwise start it
        if self:IsChecked() then
            import('/lua/ui/game/commandmode.lua').EndCommandMode(true)
        else
            import('/lua/ui/game/commandmode.lua').StartCommandMode("order", {name="RULEUCC_Ability", AbilityId = self.abilityIndex,})
        end
    elseif self.commandType == 'EACT_Toggle' then
		IssueCommand(GetUnitCommandFromCommandCap("RULEUCC_Ability"),{AbilityId = self.abilityIndex,}, false, true )
	    self:ToggleCheck()
	elseif self.commandType == 'EACT_Immediate' then
	    IssueCommand(GetUnitCommandFromCommandCap("RULEUCC_Ability"),{AbilityId = self.abilityIndex,}, false, true )
    end
end
...
local function ScriptButtonInitFunction( orderInfo, control, unitList)
    local result = nil
    local mixed = false
...
local function DroneInit( orderInfo, self, selection)
...
local function RetaliateInitFunction( orderInfo, control, unitList)
...
-- used by engineer units that can shunt or jump
local function ToggleOrderBehavior(self, modifiers)
    IssueCommand(GetUnitCommandFromCommandCap(self._order))
	self:ToggleCheck()
end

local function JumpInitFunction( orderInfo, control, unitList)
	local jumpingState = IsJumping(unitList)

    if jumpingState == -1 then
        control:SetCheck(true)
    elseif jumpingState == 1 then
        control:SetCheck(false)
    else
        control:SetCheck(false)
    end
...
local defaultOrdersTable = {
    ...
    RULEUCC_SiloBuildEMP = {        helpText = "build_nuke",    	bitmapId = 'silo-build-nuke',       preferredSlot = 7,  behavior = BuildOrderBehavior,          initialStateFunc = BuildInitFunction,},
    RULEUCC_SiloBuildAntiNuke = {   helpText = "build_nuke",    	bitmapId = 'silo-build-nuke',       preferredSlot = 7,  behavior = BuildOrderBehavior,          initialStateFunc = BuildInitFunction,},
    RULEUCC_SiloBuildRedirectNuke = {helpText = "build_nuke",    	bitmapId = 'silo-build-nuke',       preferredSlot = 7,  behavior = BuildOrderBehavior,          initialStateFunc = BuildInitFunction,},
    ...
    RULEUCC_EMP = {                 helpText = "fire_nuke",     	bitmapId = 'launch-nuke',           preferredSlot = 9,  behavior = StandardOrderBehavior, ButtonTextFunc = NukeBtnText},
	...
	RULEUCC_Jump = {				helpText = "jump",				bitmapId = 'unload',				preferredSlot = 9,  behavior = ToggleOrderBehavior, initialStateFunc = JumpInitFunction,},
	
	RULEUCC_Ability = {				helpText = "ability",   		bitmapId = 'production',            preferredSlot = 9  },
...
--to determine where they go by using preferred slots
	...
	-- Insert any ability buttons here
    for index, unit in units do
		local unitBP = unit:GetBlueprint()
        if unitBP.Abilities then
            for index, ability in unitBP.Abilities do
				if unit:IsAbilityEnabled( ability.BlueprintId ) then
                    table.insert(availableOrders, index)
					standardOrdersTable[index] = {
						preferredSlot = 8,
						bitmapId = ability.OrderBitmapId,
						abilityIndex = ability.BlueprintId,
						initialStateFunc = AbilityButtonInit,
						helpText = ability.BlueprintId,	
						behavior = AbilityButtonBehavior,
                        commandType = ability.CommandType,
					}
                end
            end
        end
