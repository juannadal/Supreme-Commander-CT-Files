...
-- Toggles the weapon range ring overlay.
function ToggleMilitary()
	local value = not Prefs.GetOption( 'allmilitary' )
	Prefs.SetOption( 'allmilitary', value )
end

-- Toggles the intel range ring overlay.
function ToggleIntel()
	local value = not Prefs.GetOption( 'allintel' )
	Prefs.SetOption( 'allintel', value )
end

-- Toggles strategic view overlay.
function ToggleStrategicView(key, value, startup)
	local value = not Prefs.GetOption( 'strategic_overlay' )
	Prefs.SetOption( 'strategic_overlay', value )
end
...
function CreateMapDropout(parent)
    ...
    local function CreateMapOptions(inMapControl)
        ...
        if camName == 'MiniMap' then
            ...
            group.toggles[1] = CreateToggleItem(group, '<LOC map_options_0002>Resources')
			group.toggles[1]:SetCheck(mapControl:IsResourceRenderingEnabled())
			group.toggles[1].OnCheck = function(self, checked)
				mapControl:EnableResourceRendering(checked)
				Prefs.SetToCurrentProfile(camName.."_resource_icons", checked)
			end
        ...
        if camName == 'WorldCamera' or camName == 'WorldCamera2' then
            ...
            group.toggles[2] = CreateToggleItem(group, wording)
            group.toggles[2]:SetCheck(false)
            group.toggles[2].OnCheck = function(self, checked)
