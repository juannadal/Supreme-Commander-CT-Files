...
function ReceiveTemplate(sender, msg)
    if Prefs.GetOption('accept_build_templates') == 'yes' then
        ...
        if tab then
            import('/lua/ui/game/announcement.lua').CreateAnnouncement('Build Template Received', tab, 'From %s', sender)
        end
...
function GetInitialName(template)
    for _, entry in template do
        ...
        return GetUnitBlueprintByName(entry[1]).Description
    end
end
...
function SendTemplate(templateID, armyIndex)
    ...
    if table.getn(templates[templateID].templateData) > 22 then
        UIUtil.QuickDialog(GetFrame(0), "You may only send build templates with 20 or less buildings.",
