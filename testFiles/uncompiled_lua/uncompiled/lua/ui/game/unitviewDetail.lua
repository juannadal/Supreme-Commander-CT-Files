...
function ShowEnhancement(bp, bpID, iconID, iconPrefix, userUnit)
    if CheckFormat() then
        ...
        if bp.Icon != nil and not string.find(bp.Name, 'Remove') then
            time, energy, mass = import('/lua/sim/game.lua').GetConstructEconomyModel(userUnit, bp)
...
function Show(bp, buildingUnit, bpID)
    if CheckFormat() then
        ...
        LayoutHelpers.AtTopIn(View.UnitShortDesc, View, 10)
        View.UnitShortDesc:SetFont(UIUtil.bodyFont, 14)
        -- modified description to use the bdID instead of the Tech level - this is a useful dev convention for prototyping - bfricks 3/13/09
        ---NOTE: remove the bpID as we get to more final UI - bfricks 3/13/09
        local description = LOCF('%s: %s', bp.BlueprintId, bp.Description)
        ...
        if buildingUnit != nil then
            local time, energy, mass = import('/lua/sim/game.lua').GetConstructEconomyModel(buildingUnit, bp.Economy)
            time = math.max(time, 1)
            showUpKeep = DisplayResources(bp, time, energy, mass)
            View.TimeStat.Value:SetFont(UIUtil.bodyFont, 14)
            View.TimeStat.Value:SetText(string.format("%s", FormatTime(time)))
...
function DisplayResources(bp, time, energy, mass)
    -- Cost Group
    if time > 0 then
        local consumeEnergy = bp.Economy.EnergyValue
        local consumeMass = bp.Economy.MassValue

    	if consumeEnergy > 0 then
    		consumeEnergy = -1.0 * consumeEnergy
    	else
    		consumeEnergy = -1.0 * energy
    	end

    	if consumeMass > 0 then
    		consumeMass = -1.0 * consumeMass
    	else
    		consumeMass = -1.0 * mass
    	end

        View.BuildCostGroup.EnergyValue:SetText( string.format("%d",consumeEnergy) )
        View.BuildCostGroup.MassValue:SetText( string.format("%d",consumeMass) )
