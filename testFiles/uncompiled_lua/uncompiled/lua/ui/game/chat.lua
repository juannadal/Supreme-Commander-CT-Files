...
local ChatTo = import('/lua/system/lazyvar.lua').Create()
        ...
        while GUI.chatLines[index] do
            ...
            if chatHistory[curEntry] then
                ...
                if curTop == 1 then
                    ...
                    GUI.chatLines[index].factionIcon:SetTexture(UIUtil.UIFile(import('/lua/common/factions.lua').Factions[chatHistory[curEntry].faction].Icon))
...
function CreateChatList(parent)
    ...
    for armyID, armyData in armies.armiesTable do
        if armyID != armies.focusArmy and armyData.human then
