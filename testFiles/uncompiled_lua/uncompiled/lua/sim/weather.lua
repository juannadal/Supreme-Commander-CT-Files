...
local WeatherDefinition = import('/lua/common/weatherdefinitions.lua')
...
function CreateWeather( inWeatherDefinition )
	ForkThread( CreateWeatherThread, inWeatherDefinition )
end

function CreateWeatherThread( inWeatherDefinition )
	local MapScale = ScenarioInfo.size -- x,z map scaling
	local WeatherDefinition = inWeatherDefinition
	local ClusterData = GetWeatherMarkerData(MapScale)
        ...
        for k, v in markers do
            ...
            if v.type == 'Weather Generator' then
                table.insert( ClusterDataList, {
					...
					cloudHeight = v.CloudHeight or 0,
