...
--  Summary  :  CollisionBeam is the simulation (gameplay-relevant) portion 
--				of a beam. It wraps a special effect that may or may not 
--				exist depending on how the simulation is executing.
...
CollisionBeam = Class(moho.CollisionBeamEntity) {
    ...
    DoDamage = function(self, instigator, damageData, targetEntity)
		...
		if not targetEntity and not radius then
			return
		end        
        ...
        elseif not instigator then
            ...
            LOG('*ERROR: Weapon = ' .. self.Weapon:GetBlueprint().BlueprintId )
        end
    end,
    ...
    OnImpact = function(self, impactType, targetEntity)
        --LOG('*DEBUG: COLLISION BEAM ONIMPACT, WEAPON = ', self.Weapon:GetBlueprint().Label, ' Type = ', impactType, ' Time = ', GetGameTimeSeconds())
