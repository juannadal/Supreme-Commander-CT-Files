--****************************************************************************
--**
--**  File     :  /lua/sim/ai/campaign/conditions/UnitCountBuildConditions.lua
...
local NumCatUnitsInArea = import('/lua/sim/ScenarioFramework.lua').NumCatUnitsInArea


CAIConditionBlueprint {
    Name = 'TargetUnitsGreaterThanOrEqual',
    Function = function(self, brain, locationName, category, number)
        local targetBrain = ArmyBrains[self.OpAI.TargetBrain]
        local numUnits = targetBrain:GetCurrentUnits( category )
        if numUnits >= number then
            return true
        end
        return false
    end,
...
CAIConditionBlueprint {
    Name = 'TargetUnitsLessThanOrEqual',
    Function = function(self, brain, locationName, category, number)
        local targetBrain = ArmyBrains[self.OpAI.TargetBrain]
        local numUnits = targetBrain:GetCurrentUnits( category )
        if numUnits <= number then
...
CAIConditionBlueprint {
    Name = 'HaveCategoryGreaterThanOrEqual',
    Function = function(self, brain, locationName, category, number)
        local numUnits = brain:GetCurrentUnits( category )
        if numUnits >= number then
            return true
        end
        return false
...
CAIConditionBlueprint {
    Name = 'HaveCategoryLessThanOrEqual',
    Function = function(self, brain, locationName, category, number)
        local numUnits = brain:GetCurrentUnits( category )
        if numUnits <= number then
            return true
...
function HaveLessThanUnitsWithCategoryInArea(aiBrain, numReq, category, area)
    local numUnits = NumCatUnitsInArea(category, ScenarioUtils.AreaToRect(area), aiBrain )
    if numUnits < numReq then
        return true
    end
    return false
...
function HaveLessThanVarTableUnitsWithCategoryInArea(aiBrain, varName, category, area)
    ...
    local numUnits = NumCatUnitsInArea(testCat, ScenarioUtils.AreaToRect(area), aiBrain )
    if ScenarioInfo.VarTable[varName] then
        if numUnits < ScenarioInfo.VarTable[varName] then
            return true
        end
