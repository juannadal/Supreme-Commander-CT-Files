...
local optionsLogic = import('/lua/ui/options/optionsLogic.lua')
local statsLogic = import('/lua/user/stats/statsLogic.lua')
...
-- read from the current stats set, find and return default if not available
function GetStat(statKey)
    local ret = GetFromCurrentProfile('stats')[statKey]
    
    if ret == nil then
        for section, secInfo in statsLogic.GetStatData() do
            for index, item in secInfo.items do
                if item.key == statKey then
                    ret = item.default
                    break
                end
            end
...
function SetStat(statKey, newValue)
    local tempStatTable = statsLogic.GetCurrent() 
    
    for i, v in tempStatTable do
        if i == statKey then
            tempStatTable[i] = newValue
            break
        end
    end
    
    statsLogic.SetCurrent(tempStatTable)
end
