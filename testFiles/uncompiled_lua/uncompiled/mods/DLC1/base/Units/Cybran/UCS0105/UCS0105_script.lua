...
UCS0105 = Class(SeaUnit) {
    ...
    OnLayerChange = function(self, new, old)
        ...
        --LOG("PZ LayerChange from <",old,"> to <",new,">")
        if( old != 'None' ) then
            if( self.AT1 ) then
                self.AT1:Destroy()
                self.AT1 = nil
     ...
     -- Just started my jump
        ...
        -- Play proper jump start based on the terrain type I came from
				if (old == "Land") then
					self:PushAnimPack( "custom_anim", 'ucs0105_JumpStart_Land', "Override" )
				elseif (old == "Water") then
					self:PushAnimPack( "custom_anim", 'ucs0105_JumpStart_Water', "Override" )
                    self:AdjustCollisionBox(true)
				end
