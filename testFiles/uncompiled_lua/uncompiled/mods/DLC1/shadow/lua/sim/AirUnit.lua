...
AirUnit = Class(MobileUnit) {
	...
	CreateDestructionPart = function(self, partInfo, scale, mesh, velocity)
		...
		Warp( proj, pos )
		proj:SetVelocity( velocity[1], velocity[2], velocity[3] )
		...
		if not self.DestructionPartsCreated then
			...
			if DestructionParts then
				...
				for k, v in DestructionParts do
					...
					if v.Mesh then
						self:CreateDestructionPart( v, scale, v.Mesh, {x,y,z})
						self:HideBone( v.AttachBone, true )
					elseif v.Meshes then
						for kMesh, vMesh in v.Meshes do
							self:CreateDestructionPart(v, scale, vMesh, {x,y,z})
						end
						self:HideBone( v.AttachBone, true )
					end
				end
