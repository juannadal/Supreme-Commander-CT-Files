...
MobileUnit = Class(Unit) {
    ...
    OnKilled = function(self, instigator, type, overkillRatio)
		...
		if layer == 'Air' then
			...
			self:DestroyJumpEffects()
            self.OverKillRatio = overkillRatio
			self.Callbacks.OnKilled:Call(self, instigator, type)
			self.DestructionExplosionWaitDelayMax = 0
    ...
    CreateUnitWaterTrailEffect = function( self )
		...
		if WaterTrailEffect and EffectTemplates.Wreckage[WaterTrailEffect]  then
			...
			for kEffect, vEffect in EffectTemplates.Wreckage[WaterTrailEffect] do
                CreateEmitterOnEntity( self, army, vEffect )
			end
		end
    end,
	...
	DestroyJumpEffects = function(self)
		if self.JumpEffects then
			for k, v in self.JumpEffects do
				v:Destroy()
			end
    ...
    LJumpAnimationHandler = function(self, sJumpStartAnim, sJumpLoopAnim)
		if (not self.Animator) then
			self.Animator = CreateAnimator(self)
		end
		self.Animator:PlayAnim(sJumpStartAnim)
		if sJumpLoopAnim != nil then
			WaitFor(self.Animator)
			self.Animator:PlayAnim(sJumpLoopAnim, true)
		end
    end,
    ...
    OnLayerChange = function(self, new, old)
		...
		if old != 'None' and old != 'Air' and not self:IsUnitState('Attached') then
			...
			-- Play Jumpstart animation if the unit has one
			if ( self.InJump and new == "Air" ) then
				local bp = self:GetBlueprint()
				if bp.Display.AnimationJumpStart then
					local jumpLoopAnim = nil
					if bp.Display.AnimationJumpLoop then
						jumpLoopAnim = bp.Display.AnimationJumpLoop
					end
					if not bp.AnimSet then
						if (old == "Land") then
							self:ForkThread(self.LJumpAnimationHandler, bp.Display.AnimationJumpStart.JS_Land, jumpLoopAnim)
						elseif (old == "Water") then
							self:ForkThread(self.LJumpAnimationHandler, bp.Display.AnimationJumpStart.JS_Water, jumpLoopAnim)
						end
					end
				end
			end
