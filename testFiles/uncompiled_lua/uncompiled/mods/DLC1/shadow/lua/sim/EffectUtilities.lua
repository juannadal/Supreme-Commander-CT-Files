...
function SpawnLavaPlumes(pos)

	WaitSeconds( 1.0 )
	-- Create an entity that will allow the spawning of projectiles, since projectiles require an
	-- entity to be created.
	local spawnEntity = Entity()
	Warp( spawnEntity, pos )
	
	local GetRandomFloat = import('/lua/system/utilities.lua').GetRandomFloat

	-- Spawn a plume projectile at an interval
	while spawnEntity do

		--LOG( 'SPAWN PLUME ', repr(pos) )
		
        local num_projectiles = GetRandomFloat( 2, 10 )

		-- For the number of projectiles we want to spawn, create several projectile plumes
		for i = 1,num_projectiles do
			
			-- Ensure that our plumes are never bunched on the X and Z plane. The following
			-- simply determines the direction of the spawned projectile.
			...
			yVec = GetRandomFloat( 1.0, 3.0 )
			zVec = math.cos(angleInitial + (i*horizontal_angle) + GetRandomFloat(-angleVariation, angleVariation) )
			px = GetRandomFloat( 0.2, 15.0 ) * xVec
			py = GetRandomFloat( 0.5, 0.9 ) * yVec
			pz = GetRandomFloat( 0.2, 15.0 ) * zVec

			-- Spawn the projectile
			local proj = spawnEntity:CreateProjectile( '/effects/Entities/LavaPlume01/LavaPlume01_proj.bp', px, py, pz, xVec, yVec, zVec )
			
			-- Set the projectiles Speed
			local velocity = GetRandomFloat( 30, 70 )
			proj:SetVelocity( velocity )
			
			-- Set the projectiles Gravity
            proj:SetBallisticAcceleration( -velocity * 0.4 )

			-- Set the lifetime of the projectile            
			proj:SetLifetime( 20 )

			-- Attach effects to the projectile.
			CreateAttachedEmitters( proj, -1, -1, EffectTemplates.Marker.D1_201Lava01 )
		end		
